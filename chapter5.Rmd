...
## How doe each of N & M relate to K
...
n_seq <- seq(-2,2, length.out = 100)
mu_N <- link(mN, data = data.frame(N=n_seq))
mu_N_mean <- apply(mu_N, 2, mean)
mu_N_PI <- apply(mu_N,2, PI)


# Counterfactual plot
cf_data <- data.frame(N=n_seq, M=0)
mu_CF_M0 <- link(mNM, data = cf_data)
mu_CF_M0_mean <- apply(mu_CF_M0, 2, mean)
mu_CF_M0_PI <- apply(mu_CF_M0, 2, PI)

par(mfrow=c(1,2))
plot(K ~ N, data=dcc, col=rangi2,
     xlab='neocortex percent (std)',
     ylab='kilocal per gram (std)')
lines(n_seq, mu_N_mean)
shade(mu_N_PI, n_seq)

plot(NULL, xlab='neocortex percent (std)', ylab='kilocal per gram (std)',
     xlim=range(n_seq), ylim=c(-1,2))
lines(n_seq, mu_CF_M0_mean)
shade(mu_CF_M0_PI, n_seq)
...
mu_M <- link(mM, data = data.frame(M=m_seq))
mu_M_mean <- apply(mu_M, 2, mean)
mu_M_PI <- apply(mu_M,2, PI)


# Counterfactual plot
cf_data <- data.frame(M=m_seq, N=0)
mu_CF_N0 <- link(mNM, data = cf_data)
mu_CF_N0_mean <- apply(mu_CF_N0, 2, mean)
mu_CF_N0_PI <- apply(mu_CF_N0, 2, PI)

par(mfrow=c(1,2))
plot(K ~ M, data=dcc, col=rangi2,
     xlab='log body mass (std)',
     ylab='kilocal per gram (std)')
lines(m_seq, mu_M_mean)
shade(mu_M_PI, m_seq)

plot(NULL, xlab='log body mass (std)', ylab='kilocal per gram (std)',
     xlim=range(m_seq), ylim=c(-1,2))
lines(m_seq, mu_CF_N0_mean)
shade(mu_CF_N0_PI, m_seq)
...
## Simulating DAGs for Masking pattern
...
plot( coeftab(mN_sim, mM_sim, mNM_sim))
